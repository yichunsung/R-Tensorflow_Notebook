---
title: "Session creation"
author: "YiChun Sung"
date: "2017-09-01"
output: 
  html_document:
    highlight: zenburn
    theme: readable
    toc: yes
    toc_float:
      collapsed: no
      smooth_scroll: yes
---


## Session

In Tensorflow, we use the session function to create a object to control Tensorflow deep learning model. After you build your model, you can use session to start and close your Tensorflow model.

```{r eval=FALSE}
library(tensorflow)
sess <- tf$Session()
sess$run()
sess$close()
```

## Demo

Let's code a demo to show you how to use `tf$Session()`.

In this case, I create two matrix and use Tensorflow function `tf$matmul()` to multiply them together.

`run()` your model and `close()` it.
```{r}
library(tensorflow)
# Matrix 1 and Matrix 2
matrix_1 <- matrix(3, 1,2)
matrix_2 <- matrix(2, 2,1)

product <- tf$matmul(matrix_1, matrix_2)

sess <- tf$Session()
result <- sess$run(product)
print(result) #[12]
sess$close()

```